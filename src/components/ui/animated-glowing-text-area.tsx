...
const AnimatedGlowingTextArea = React.forwardRef<
  HTMLTextAreaElement,
...
>(({ className, ...props }, ref) => {
  return (
    <div className="relative group">
      <div
        className="absolute -inset-0.5 rounded-xl bg-primary opacity-0 group-hover:opacity-75 group-focus-within:opacity-75 blur transition duration-1000"
      ></div>
      <div
        className="absolute z-[-1] overflow-hidden h-full w-full rounded-xl blur-[3px] 
                        before:absolute before:content-[''] before:z-[-2] before:w-[999px] before:h-[999px] before:bg-no-repeat before:top-1/2 before:left-1/2 before:-translate-x-1/2 before:-translate-y-1/2 before:rotate-60
                        before:bg-[conic-gradient(hsl(var(--background)),hsl(var(--primary))_5%,hsl(var(--background))_38%,hsl(var(--background))_50%,hsl(var(--accent))_60%,hsl(var(--background))_87%)]
      ...
      <div
        className="absolute z-[-1] overflow-hidden h-full w-full rounded-xl blur-[3px] 
                        ...
                        before:bg-[conic-gradient(rgba(0,0,0,0),hsl(var(--primary)),rgba(0,0,0,0)_10%,rgba(0,0,0,0)_50%,hsl(var(--accent)),rgba(0,0,0,0)_60%)]
                        before:transition-all before:duration-2000
      ...
      <Textarea
        ref={ref}
        className={cn(
          'relative bg-background/90 border-none w-full min-h-[120px] rounded-lg text-foreground px-4 py-3 text-base focus:outline-none placeholder:text-muted-foreground resize-none',
          className
        )}
        {...props}
      />
